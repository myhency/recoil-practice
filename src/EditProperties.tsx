...
const editSize = selectorFamily<any, {dimension: 'width' | 'height'; id: number}>({
    ...
    set:
        ...
        ({get, set}, newValue) => {
            ...
            if (dimension === 'width') {
                set(size, {width: newValue, height: Math.round(newValue / aspectRatio)})
            } else {
                set(size, {width: Math.round(newValue * aspectRatio), height: newValue})
            }
        },
})
...
const Property = ({label, path, id}: {label: string; path: string; id: number}) => {
    const [value, setValue] = useRecoilState<number>(editPropertyState({path, id}))
    if (value === null) return null
    return <PropertyInput label={label} value={value} onChange={setValue} />
}
